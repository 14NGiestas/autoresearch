...
!
! WHY THIS EXISTS: energy used to be measured from the OUTSIDE (scripts/energy_run.py
! plus energy/ledger.jsonl) and pasted into the card afterwards with a LINEAR
! per-step apportionment (J_job x ckpt_steps/job_steps). Measuring INSIDE the
! process makes each checkpoint's number exact by construction: the J spent
! between the previous save and this one. The field energy.self_measured='1'
! says the number came from here.
!
! MINIMAL USE (5 lines):
...
!     ... work ...
!     call energy_mark('train', tokens=n)      ! J/wall/cpu/IO OF THAT phase
...
!             helps interpret it (e.g. big checkpoint vs training step).
!
...
! cpu_s, IO) under `label`. The label names the phase that JUST happened, which is
! how people use it: call energy_mark('checkpoint') right after saving. Repeated
! calls with the same label ACCUMULATE (n counts how many).
!
! NOT thread-safe (counter/integrator/phase state): call it from the main thread.
! The trainer calls it at saves and at logs, never inside an OpenMP region.
!
! These are the three things the module measures, in one call each:
!   energy_joules()      J since init (or since the previous mark/interval call)
...
module fortran_energy_mod
  ...
  ! One measured interval: what a phase/save/checkpoint consumed since that mark.
  ! It is ALSO the complete record a consumer needs -- log line, checkpoint card,
...
contains
  ...
  ! building block of energy_mark and what the trainer uses per checkpoint.
  ...
  !   * a progress trace that must not disturb the per-checkpoint interval;
  !   * reading the numbers twice (report + card) without the second read
